# -*- coding: utf-8 -*-
"""
Created on Thu Jul  9 11:05:29 2020
...
def checkconstraint(a1,a2,a3,a4,a5,a6,a7,a8,a9,a10,a11,a12,
    ...
    ahmed=np.array([[a1,a2,a3,a4,a5,a6,a7,a8,a9,a10,a11,a12],
                    [part1max,part1max,part1max,part1max,part1max,part1max,part1max,part1max,part1max,part1max,part1max,part1max]])
    bine=np.array([[d1,d2,d3,d4,d5,d6,d7,d8,d9,d10,d11,d12],
                   [part2max,part2max,part2max,part2max,part2max,part2max,part2max,part2max,part2max,part2max,part2max,part2max]])
    hassan=np.array([[g1,g2,g3,g4,g5,g6,g7,g8,g9,g10,g11,g12],
                     [part3max,part3max,part3max,part3max,part3max,part3max,part3max,part3max,part3max,part3max,part3max,part3max]])
    moullay=np.array([[j1,j2,j3,j4,j5,j6,j7,j8,j9,j10,j11,j12],
                      [part4max,part4max,part4max,part4max,part4max,part4max,part4max,part4max,part4max,part4max,part4max,part4max]])
    almassira=np.array([[m1,m2,m3,m4,m5,m6,m7,m8,m9,m10,m11,m12],
                        [part5max,part5max,part5max,part5max,part5max,part5max,part5max,part5max,part5max,part5max,part5max,part5max]])
    return (np.all(ahmed[1]>=ahmed[0]) and np.all(bine[1]>=bine[0]) and np.all(hassan[1]>=hassan[0]) and np.all(moullay[1]>=moullay[0]) and np.all(almassira[1]>=almassira[0]))
...
def ribasimmodel(c1,c2,c3,c4,c5,c6,c7,c8,c9,c10,c11,c12,
 ...
 #230.70
    part5max=285.00                    
                       ...
                       #m1,m2,m3,m4,m5,m6,m7,m8,m9,m10,m11,m12,
                       #part1max,part2max,part3max,part4max,part5max):
    f = open("C:\\Ribasim7\\OUMRBIA9.Rbn\\Actions\\Measures\\R001-2015-RibasimRsvNodeOperRulesDataOER.mes", 'r+')
    a=f.readlines()
    ...
    f = open("C:\\Ribasim7\\OUMRBIA9.Rbn\\Actions\\Measures\\R001-2015-RibasimRsvNodeOperRulesDataOER.mes", 'wt')
    f.seek(0)
    f.writelines(a)
...
#break point
    ...
    os.chdir("C:\\Oer0T\\OUMRBIA9.Rbn\\CMTWORK")
    resrb = subprocess.call("C:\\Oer0T\\Programs\\Ribasim\\System\\Bin2prt.exe BIN2PRT.fnm") #Doubt
    if resrb <0 and resrb> 0:
        print('Error in RIBASIM:')
        print(resrb)
        exit()
    resrb = subprocess.call("C:\\Oer0T\\Programs\\Ribasim\\System\\Simproc.exe C:\\Oer0T\\OUMRBIA9.Rbn\\CMTWORK\\simproc.fnm") #Doubt
    if resrb <0 and resrb> 0:
        print('Error in RIBASIM:')
        print(resrb)
        exit()
    resrb = subprocess.call("C:\\Oer0T\\Programs\\Runlist\\RUNLIST.EXE  C:\\Oer0T\\Programs\\RL_PSTPR.INI")
    if resrb <0 and resrb> 0:
        print('Error in postprocessing RIBASIM')
        print(resrb)
        exit()
    reshis = HisFile("C:\\Oer0T\\OUMRBIA9.Rbn\\WORK\\pws.his")
    reshis.read()
    ...
    reshis1 = HisFile("C:\\Oer0T\\OUMRBIA9.Rbn\\WORK\\reservoi.his")
    reshis1.read()
    ...
    pwssum=sum(pws1)+sum(pws2)+sum(pws3)+sum(pws4)+sum(pws5)+sum(pws6)+sum(pws7)+sum(pws8)+sum(pws9)+sum(pws10)+sum(pws11)+sum(pws12)+sum(pws13)+sum(pws14)+sum(pws15)+sum(pws16)+sum(pws17)+sum(pws18)+sum(pws19)+sum(pws20)+sum(pws21)
    energy1=sorted(datatagzirt)
    energy1=sum(energy1[880:-1])/len(energy1[880:-1])
    energy2=sorted(datahassan)
    energy2=sum(energy2[880:-1])/len(energy2[880:-1])
    energy3=sorted(datatyoughza)
    energy3=sum(energy3[880:-1])/len(energy3[880:-1])
    energy4=sorted(datamoulayyoussef)
    energy4=sum(energy4[880:-1])/len(energy4[880:-1])
    energy5=sorted(dataalmassira)
    energy5=sum(energy5[880:-1])/len(energy5[880:-1])
    energy6=sorted(databinelouidane)
    energy6=sum(energy6[880:-1])/len(energy6[880:-1])
    energy7=sorted(dataahmedhansal)
    energy7=sum(energy7[880:-1])/len(energy7[880:-1])
    energy8=sorted(datasididriss)
    energy8=sum(energy8[880:-1])/len(energy8[880:-1])
    energyaverage=(energy1+energy2+energy3+energy4+energy5+energy6+energy7+energy8)/8
    return pwsshortageavg, energyaverage, pwssum
...
#for i in range(basin_model.time_horizon):
    ...
    basin_model.outcomes = [ScalarOutcome('pwsshortageavg', kind=ScalarOutcome.MINIMIZE),
    ...
    with SequentialEvaluator(basin_model) as evaluator:
        ...
        results, convergence=evaluator.optimize(nfe=100, searchover='levers',epsilons=[1, 1, 1], convergence=convergence_metrics, reference=None)#constraints=m)
...
plt.set_ylabel('$\epsilon$-progress')


plt.set_xlabel('number of function evaluations')
