...
class RibasimModel(SingleReplication, AbstractModel):
    ...
    def model_init(self, policy):
        ...
        if not self.directory_setup:
            ...
            update_paths(files, '\Oer0T_1', self.working_directory[2:])
            ...
            update_paths(files, 'C:\Oer0T_1', self.working_directory)

            files = [os.path.join(self.working_directory, 'Programs', 'rib2his4.fnm')]
            update_paths(files, 'C:\Oer0T_1', self.working_directory)
            self.directory_setup = True
...
if __name__ == '__main__':
    ...
    basin_model = RibasimModel('ribasim', original_dir="C:\\Oer0T_2")
    ...
    basin_model.outcomes = [ScalarOutcome('pwsobjective', kind=ScalarOutcome.MINIMIZE,expected_range=(1,33.3)),
                            ScalarOutcome('energyobjective', kind=ScalarOutcome.MAXIMIZE,expected_range=(641,3291)),
                            ScalarOutcome('virobjective',kind=ScalarOutcome.MINIMIZE,expected_range=(315,1341))]
                            #ScalarOutcome('firobjective', kind=ScalarOutcome.MINIMIZE),
                            
    

    convergence_metrics = [HyperVolume.from_outcomes(basin_model.outcomes),EpsilonProgress()]
